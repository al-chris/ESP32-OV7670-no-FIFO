...
bool I2SCamera::initVSync(int pin)
{
  ...
  // Register ISR first, then enable the GPIO interrupt to avoid missing the first edge.
...
{    
  ...
  for (int i = 0; i < sizeof(pins) / sizeof(pins[0]); ++i) {
    ...
    // Allocate I2S interrupt, keep it disabled. Fail cleanly if allocation fails.
    esp_err_t _res = esp_intr_alloc(ETS_I2S0_INTR_SOURCE, ESP_INTR_FLAG_INTRDISABLED | ESP_INTR_FLAG_LEVEL1 | ESP_INTR_FLAG_IRAM, &I2SCamera::i2sInterrupt, NULL, &i2sInterruptHandle);
    if (_res != ESP_OK) {
